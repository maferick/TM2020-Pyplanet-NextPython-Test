...
class ChatQuery(Query):
	...
	def to_players(self, *players):
		...
		if len(players) == 1 and isinstance(players[0], collections.abc.Iterable):
			players = players[0]
		...
		elif isinstance(players, collections.abc.Iterable) and isinstance(players, collections.abc.Sized):
			self._logins = set()
			self.add_to(players)
		return self
