...
class PlayerManager(CoreContrib):
	...
	async def map_loaded(self, *args, **kwargs):
		"""
		Reindex the current number of players and spectators, and reconcile the
		online player set against the dedicated server's authoritative player list.

		gbx callbacks are dispatched concurrently, so a quick disconnect/reconnect can
		leave the online set with a ghost (missed disconnect) or a missing player
		(missed connect). Reconciling here makes any such desync self-heal each map.
		Note: handle_connect/handle_disconnect only update internal manager state, they
		do not emit the player connect/disconnect signals, so this won't spam the apps.
		...
		# Reconcile the online set with reality (self-heal connect/disconnect desync).
		ghost_logins = set(self._online_logins) - live_logins
		missing_logins = live_logins - set(self._online_logins)
		# Safety: if the server returned no players while we still track some online,
		# the list is almost certainly a transient glitch during the map switch - skip
		# the ghost pass so we never wrongly drop the entire online set.
		if not live_logins and self._online_logins:
			ghost_logins = set()
		for login in ghost_logins:
			try:
				await self.handle_disconnect(login)
			except Exception as e:
				logging.warning('Player desync reconcile: failed to drop ghost {}: {}'.format(login, e))
			# Force-prune in case handle_disconnect early-returned (cache already gone).
			self._online_logins.discard(login)
			self._online = {p for p in self._online if getattr(p, 'login', None) != login}
		for login in missing_logins:
			try:
				await self.handle_connect(login)
			except Exception as e:
				logging.warning('Player desync reconcile: failed to add missing {}: {}'.format(login, e))
