...
class PyPlanetConfig(AppConfig):
	...
	def __init__(self, *args, **kwargs):
		...
		self.controller_view = ControllerView(self, manager=self.context.ui)

		# Toggleable controller branding (off by default), configurable via //settings.
		self.setting_show_logo = Setting(
			'show_logo', 'Show PyPlanet logo', Setting.CAT_BEHAVIOUR, type=bool,
			description='Show the small PyPlanet logo in the bottom-right corner of the screen.',
			default=False, change_target=self.reload_controller_view
		)
		self.setting_show_report = Setting(
			'show_report_link', 'Show "Report issue" link', Setting.CAT_BEHAVIOUR, type=bool,
			description='Show the "Report issue / suggestion" link in the controller overlay.',
			default=False, change_target=self.reload_controller_view
		)
	...
	async def reload_controller_view(self, *args, **kwargs):
		# Re-render the overlay so a toggled setting takes effect immediately.
	...
	async def on_start(self):
		...
		# Register the branding toggles and render the global overlay.
		await self.context.setting.register(self.setting_show_logo, self.setting_show_report)
		await self.controller_view.display()
	...
	async def chat_upgrade(self, player, data, *args, **kwargs):
		...
		run_detached(self.send_updates())
