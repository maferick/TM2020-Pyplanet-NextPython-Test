...
async def handle_scores(source, signal, **kwargs):
	async def get_player_scores(data):
		# A player present in the dedicated's score list may already have disconnected
		# (and been removed from PyPlanet's cache). Resolve without the 4s retry-lock and
		# tolerate a missing player so one disconnect can't drop the whole scores batch.
		try:
			player = await Controller.instance.player_manager.get_player(login=data['login'], lock=False)
	...
	# Drop any entries that still failed to build so a single bad score can't void the batch.
	clean_player_scores = []
	for entry in player_scores:
		if isinstance(entry, BaseException):
			logger.warning('Skipping a player score entry that failed to build: %r', entry)
			continue
		clean_player_scores.append(entry)
	player_scores = clean_player_scores
	team_scores = [
