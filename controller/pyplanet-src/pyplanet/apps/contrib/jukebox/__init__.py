...
class Jukebox(AppConfig):
	...
	async def on_start(self):
		...
		# Advance the jukebox at end-of-map. We use the native EndMap callback
		# (fires once per map regardless of game mode/podium) instead of
		# flow.podium_start, which never fires for modes without a podium and can
		# fire twice (map + match podium), skipping a juked map. (upstream #1278)
